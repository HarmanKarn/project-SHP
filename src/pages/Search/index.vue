...
<script>
  ...
  export default {
    ...
    beforeMount(){
      ...
      //在发送请求之前,把接口需要传递参数,进行整理(在给服务器发请求之前,把参数整理好,服务器就会返回查询的对象)
    ...
    computed:{
      ...
      //mapGetters里面的写法:传递的数组,因为getters计算式没有划分模块[home,search]
    ...
    methods:{
      ...
      removeCategoryName(){
        ...
        //把相应的字段设置为undefined,当前这个字段不会带给服务器
        ...
        this.getData();
        //地址栏也需要修改:进行路由跳转
        //删除query,如果路径当中出现params参数,路由跳转的时候应该带着params参数
      ...
      //排序的操作
      changeOrder(flag){
        //flag形参:标记,代表用户点击的是综合(1)或价格(2) 用户点击的时候传递进来的
        ...
        //将newOrder赋值给searchParams.order
    ...
    watch:{
      ...
      $route(){
        ...
        //再次发起ajax请求
        this.getData()
        //每一次请求完毕,应该把相应的1,2,3级分类的id设置为空,接收下一次相应的1,2,3级分类的id
        //分类名字和关键字不需要清理,因为每次路由发生变化的时候,都会给他赋予新的数据
        this.searchParams.category1Id = undefined;
        this.searchParams.category2Id = undefined;
        this.searchParams.category3Id = undefined;
      }
